...
- [ ] **C5** `redactConnStrings` leaks the password tail when the password contains a raw `@` (stops at first `@`; RFC/Postgres use the last). `agent-core/src/db/connstr.ts:38`. Fix: userinfo run greedy to the LAST `@` before the host.
...
- [ ] **H5** Catastrophic O(n^2) backtracking in `redactConnStrings` hangs main on one long line (~97s on 400k chars). `db/connstr.ts:38`. Fix: non-overlapping / length-bounded userinfo class.
