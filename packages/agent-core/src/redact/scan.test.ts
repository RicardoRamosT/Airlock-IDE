...
describe("scanForSecrets", () => {
  it("finds a vaulted value literally and names it (1-indexed line)", () => {
    ...
    const f = scanForSecrets(text, [
      { name: "API_KEY", value: "supersecretvalue" },
    ]);
  ...
  it("flags known patterns by type, even when not vaulted", () => {
    ...
    const f = scanForSecrets(text, []);
    expect(f).toEqual([
      { line: 1, kind: "pattern", patternType: "stripe-secret" },
    ]);
  });
  ...
  it("flags a PEM private key header", () => {
    ...
    expect(f).toEqual([
      { line: 1, kind: "pattern", patternType: "pem-private-key" },
    ]);
  });
  ...
  it("dedupes per line + identity", () => {
    ...
    const f = scanForSecrets(text, [
      // biome-ignore lint/suspicious/noSelfCompare: intentional constant -- forces "tok " (4 chars) without a bare string literal
      // biome-ignore lint/correctness/noConstantCondition: same
      { name: "T", value: "tok" === "tok" ? "tok " : "" },
    ]);
