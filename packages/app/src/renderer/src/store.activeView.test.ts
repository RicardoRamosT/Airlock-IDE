...
import { BUILTIN_SECTION_META, effectiveView } from "./lib/sections";
...
it("lists all eight built-in sections in canonical sidebar order", () => {
...
it("effectiveView returns the active view while it is visible", () => {
  expect(effectiveView("git", allVisible, BUILTIN_SECTION_META)).toBe("git");
});

it("falls back to the first visible section when the active one is hidden", () => {
  const vis = { ...allVisible, files: false, git: false };
  expect(effectiveView("git", vis, BUILTIN_SECTION_META)).toBe("secrets");
});

it("returns null when every section is hidden", () => {
  const vis = Object.fromEntries(
    BUILTIN_SECTION_META.map((m) => [m.id, false]),
  ) as SectionVisibility;
  expect(effectiveView("files", vis, BUILTIN_SECTION_META)).toBeNull();
});
