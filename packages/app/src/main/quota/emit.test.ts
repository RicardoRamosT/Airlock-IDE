...
const EMIT = fileURLToPath(
  new URL("../../../resources/statusline-emit.cjs", import.meta.url),
);
let dir: string;
...
function run(input: string, cfg: object) {
  ...
  return spawnSync(process.execPath, [EMIT, cfgPath], {
    input,
    encoding: "utf8",
  });
}
...
it("chains a prior command and passes its stdout through", () => {
  const out = path.join(dir, "rate-limits.json");
  const r = run("hello-footer", {
    out,
    prior: { type: "command", command: "cat" },
  });
